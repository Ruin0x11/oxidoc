...
pub type DocRelatedItems = HashMap<DocType, Vec<StoreLocation>>;
...
impl Documentation {
    ...
    fn subitems(&self) -> String {
        let categories = match self.inner_data {
            ...
            DocInnerData::EnumDoc(..) => vec![DocType::Function, DocType::Variant],
            _ => vec![],
        };

        categories
            .iter()
            .map(|c| self.subitems_in_category(c))
            .filter(|c| c.is_some())
            .map(|c| c.unwrap())
            .collect::<Vec<String>>()
            .join("\n\n")
    ...
    pub fn save(&self) -> Result<()> {
...

        {
            ...
            fs::create_dir_all(parent_path).chain_err(|| {
