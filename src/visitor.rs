...
impl OxidocVisitor {
    ...
    fn visit_const(&self, item: &ast::Item, ast_ty: &ast::Ty, ast_expr: &ast::Expr) -> Constant {
        Constant {
            ident: item.ident,
            ...
            items: trait_items
                ...
                .map(|ti| {
                    TraitItem {
                        ...
                        path: ModPath::join(
                            &self.current_scope.append_ident(item.ident),
    ...
    fn add_uses(&self, module: &mut Module, _item: &ast::Item, import: &ast::ViewPath) {
            ...
            ast::ViewPath_::ViewPathList(ref path, ref items) => {
                for item in items {
                    ...
                    let full_path = if path.segments.last().unwrap().identifier == ident {
                        ...
                        ModPath::join(&ModPath::from(path.clone()), &ModPath::from(ident))
                    };
    ...
    fn visit_item(&mut self, item: &ast::Item, module: &mut Module) {
        match item.node {
            ...
            ast::ItemKind::Fn(ref decl, unsafety, constness, abi, ref generics, _) => {
                let f = self.visit_fn(item, &*decl, unsafety, constness.node, abi, generics);
                module.fns.push(f);
            }
            ast::ItemKind::Mod(ref mod_) => {
                let m = self.visit_module(item.attrs.clone(), mod_, Some(item.ident));
                module.mods.push(m);
            }
            ast::ItemKind::Enum(ref def, ref generics) => {
                let e = self.visit_enum_def(item, def, generics);
                module.add_use(&item.ident, e.path.clone());
                module.enums.push(e);
            }
            ast::ItemKind::Struct(ref variant_data, ref generics) => {
                let s = self.visit_struct(item, variant_data, generics);
            ...
            ast::ItemKind::Trait(unsafety, ref generics, ref param_bounds, ref trait_items) => {
                let t = self.visit_trait(item, unsafety, generics, trait_items);
                module.traits.push(t);
            }
            ast::ItemKind::DefaultImpl(unsafety, ref trait_ref) => {
                let def_trait = self.visit_default_impl(item, unsafety, trait_ref);
            ...
            ast::ItemKind::Impl(unsafety,
                                ...
                                ref ty,
                                ref items) => {
                let i = self.visit_impl(item, unsafety, generics, trait_ref, ty, items);
                module.impls.push(i);
            }
            ast::ItemKind::Ty(..) |
            ast::ItemKind::Static(..) |
    ...
    pub fn visit_crate(&mut self, krate: ast::Crate) {
        ...
        self.crate_module = self.visit_module(krate.attrs.clone(), &krate.module, None);
